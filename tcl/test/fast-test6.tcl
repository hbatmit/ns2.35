...
proc create_tcp_fast { idx src dst start stop } {
	...
	$tcps($idx) set mi_threshold_ 0.006
...
proc record {} {
	...
	for {set i 0} {$i < $ntcps} {incr i} {
		...
		if { ($now > [expr $SimDuration/2]) && ($have_reset_mi_threshold < 1) } {
			$tcps($i) set mi_threshold_ 0.001
			set have_reset_mi_threshold 1
		}
	}
...
# Predefine tracing
set Name "fast-test6-record.dat";
set Q1 "fast-test6-queue1.dat";
set Q2 "fast-test6-queue2.dat";
set f0 [open $Name w]
set f1 [open $Q1 w]
set f2 [open $Q2 w]
