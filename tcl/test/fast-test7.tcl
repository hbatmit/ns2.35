...
proc create_tcp_fast { idx src dst start stop } {
	...
	$tcps($idx) set alpha_ 10
	$tcps($idx) set beta_ 10
	$tcps($idx) set mi_threshold_ 0.0001
	$tcps($idx) set useHeaders_ false
	$tcps($idx) set high_accuracy_cwnd_ 0
...
proc record {} {
	...
	global have_reset_mi_threshold SimDuration
	...
	for {set i 0} {$i < $ntcps} {incr i} {
		...
		if { ($now > [expr $SimDuration/3]) && ([$tcps($i) set high_accuracy_cwnd_] < 1) } {
			$tcps($i) set high_accuracy_cwnd_ 1
		}
		if { ($now > [expr 2*$SimDuration/3]) && ([$tcps($i) set high_accuracy_cwnd_] < 2) } {
			$tcps($i) set high_accuracy_cwnd_ 2
		}
	}
...
# Predefine tracing
set Name "fast-test7-record.dat";
set Q1 "fast-test7-queue1.dat";
set Q2 "fast-test7-queue2.dat";
set f0 [open $Name w]
set f1 [open $Q1 w]
set f2 [open $Q2 w]
