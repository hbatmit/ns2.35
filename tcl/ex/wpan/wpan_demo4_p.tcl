...
if { ("$val(traffic)" == "mix") || ("$val(traffic)" == "cbr") || ("$val(traffic)" == "poisson") } {
   ...
   $ns_ at [expr $appTime1 + 2.0] "Mac/802_15_4 wpanCmd link-down 13 16"
   set tmpTime [format "%.1f" [expr $appTime1 + 2.0]]
   $ns_ at [expr $appTime1 + 2.0] "$ns_ trace-annotate \"(at $tmpTime) link down: 13 -> 16\""
   $ns_ at [expr $appTime2 + 3.0] "$node_(20) node-down"
   set tmpTime [format "%.1f" [expr $appTime2 + 3.0]]
   $ns_ at [expr $appTime2 + 3.0] "$ns_ trace-annotate \"(at $tmpTime) node down: 20\""
   $ns_ at [expr $appTime1 + 8.0] "Mac/802_15_4 wpanCmd link-up 13 16"
   set tmpTime [format "%.1f" [expr $appTime1 + 8.0]]
   $ns_ at [expr $appTime1 + 8.0] "$ns_ trace-annotate \"(at $tmpTime) link up: 13 -> 16\""
   $ns_ at [expr $appTime2 + 10.0] "$node_(20) node-up"
   set tmpTime [format "%.1f" [expr $appTime2 + 10.0]]
   $ns_ at [expr $appTime2 + 10.0] "$ns_ trace-annotate \"(at $tmpTime) node up: 20\""
   if { "$val(traffic)" == "cbr" } {
   	set pktType cbr
   	set pktType2 cbr
...
if { "$val(traffic)" == "ftp" } {
   ...
   puts [format "Acknowledgement for data: %s" [Mac/802_15_4 wpanCmd ack4data]]
   ftptraffic 3 18 $appTime1
   ftptraffic 9 17 $appTime2
   Mac/802_15_4 wpanNam FlowClr -p AODV -c tomato
   Mac/802_15_4 wpanNam FlowClr -p ARP -c green
   Mac/802_15_4 wpanNam FlowClr -p MAC -c navy
   $ns_ at $appTime1 "$node_(3) add-mark m1 blue circle"
   $ns_ at $appTime1 "$node_(18) add-mark m2 blue circle"
   $ns_ at $appTime1 "$ns_ trace-annotate \"(at $appTime1) ftp traffic from node 3 to node 18\""
   Mac/802_15_4 wpanNam FlowClr -p tcp -s 3 -d 18 -c blue
   Mac/802_15_4 wpanNam FlowClr -p ack -s 18 -d 3 -c blue
   $ns_ at $appTime2 "$node_(9) add-mark m3 green4 circle"
   $ns_ at $appTime2 "$node_(17) add-mark m4 green4 circle"
   $ns_ at $appTime2 "$ns_ trace-annotate \"(at $appTime2) ftp traffic from node 9 to node 17\""
   Mac/802_15_4 wpanNam FlowClr -p tcp -s 9 -d 17 -c green4
   Mac/802_15_4 wpanNam FlowClr -p ack -s 17 -d 9 -c green4
}
