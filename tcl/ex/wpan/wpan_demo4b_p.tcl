...
if { ("$val(traffic)" == "cbr") || ("$val(traffic)" == "poisson") } {
   ...
   $ns_ at [expr $appTime1 + 2.0] "Mac/802_15_4 wpanCmd link-down 7 9"
   set tmpTime [format "%.1f" [expr $appTime1 + 2.0]]
   $ns_ at [expr $appTime1 + 2.0] "$ns_ trace-annotate \"(at $tmpTime) link down: 7 -> 9\""
   ...
   $ns_ at [expr $appTime1 + 8.0] "Mac/802_15_4 wpanCmd link-up 7 9"
   set tmpTime [format "%.1f" [expr $appTime1 + 8.0]]
   $ns_ at [expr $appTime1 + 8.0] "$ns_ trace-annotate \"(at $tmpTime) link up: 7 -> 9\""
