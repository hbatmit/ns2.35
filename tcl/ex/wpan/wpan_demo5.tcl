...
set val(tr)		wpan_demo5.tr
set val(nam)		wpan_demo5.nam
...
Phy/WirelessPhy set CSThresh_ $dist(9m)
...
#set cskip3 [Agent/ZBR CSkip 3]
#set cskip4 [Agent/ZBR CSkip 4]
#set cskip5 [Agent/ZBR CSkip 5]
#set cskip6 [Agent/ZBR CSkip 6]
#puts [format "Cm:        %s" $Cm]
#puts [format "Lm:        %s" $Lm]
#puts [format "BlockSize: %s" $BlockSize]
#$ns_ at 0.0 "$ns_ trace-annotate \"Cm = $Cm, Lm = $Lm, BlockSize = $BlockSize, CSkip0 = $cskip0, CSkip1 = $cskip1, CSkip2 = $cskip2, CSkip3 = $cskip3, CSkip4 = $cskip4, CSkip5 = $cskip5, CSkip6 = $cskip6\""

source ./wpan_demo5.scn
...
$ns_ at 0.0	"$node_(0) sscs startCTPANCoord 0"	;# startCTPANCoord <txBeacon=1> <BO=3> <SO=3>

$ns_ at 0.5	"$node_(1) sscs startCTDevice"  	;# startCTDevice <isFFD=1> <assoPermit=1> <txBeacon=0> <BO=3> <SO=3>
$ns_ at 0.8	"$node_(2) sscs startCTDevice"
$ns_ at 1.3	"$node_(3) sscs startCTDevice"
$ns_ at 1.8	"$node_(4) sscs startCTDevice"

$ns_ at 2.3	"$node_(5) sscs startCTDevice 1 1 1"
$ns_ at 2.4	"$node_(6) sscs startCTDevice 1 1 1"
$ns_ at 2.5	"$node_(7) sscs startCTDevice 1 1 1"
$ns_ at 2.6	"$node_(8) sscs startCTDevice 1 1 1"

$ns_ at 3.3	"$node_(9) sscs startCTDevice"
$ns_ at 3.8	"$node_(10) sscs startCTDevice"
$ns_ at 3.3	"$node_(11) sscs startCTDevice"
$ns_ at 3.8	"$node_(12) sscs startCTDevice"

# ---

for {set i 13} {$i < 17} {incr i} {
	$ns_ at 4.3	"$node_($i) sscs startCTDevice"
}
for {set i 17} {$i < 21} {incr i} {
	$ns_ at 5.3	"$node_($i) sscs startCTDevice"
}
for {set i 21} {$i < 25} {incr i} {
	$ns_ at 6.3	"$node_($i) sscs startCTDevice"
}

# ---

for {set i 25} {$i < 29} {incr i} {
	$ns_ at 7.3	"$node_($i) sscs startCTDevice 1 1 1"
}
for {set i 29} {$i < 33} {incr i} {
	$ns_ at 7.3	"$node_($i) sscs startCTDevice"
}
for {set i 33} {$i < 41} {incr i} {
	$ns_ at 8.3	"$node_($i) sscs startCTDevice"
}

# ---

for {set i 41} {$i < 49} {incr i} {
	$ns_ at 9.3	"$node_($i) sscs startCTDevice"
}
for {set i 49} {$i < 57} {incr i} {
	$ns_ at 10.3	"$node_($i) sscs startCTDevice"
}
for {set i 57} {$i < 61} {incr i} {
	$ns_ at 11.3	"$node_($i) sscs startCTDevice"
}

# ---

for {set i 61} {$i < 65} {incr i} {
	$ns_ at 12.3	"$node_($i) sscs startCTDevice 0"
}
for {set i 65} {$i < 73} {incr i} {
	$ns_ at 12.3	"$node_($i) sscs startCTDevice"
}
for {set i 73} {$i < 77} {incr i} {
	$ns_ at 13.3	"$node_($i) sscs startCTDevice 0"
}
for {set i 77} {$i < 80} {incr i} {
	$ns_ at 13.3	"$node_($i) sscs startCTDevice"
}
for {set i 80} {$i < 85} {incr i} {
	$ns_ at 13.3	"$node_($i) sscs startCTDevice 0"
}

# ---

for {set i 85} {$i < 93} {incr i} {
	$ns_ at 14.3	"$node_($i) sscs startCTDevice 0"
}
for {set i 93} {$i < 101} {incr i} {
	$ns_ at 15.3	"$node_($i) sscs startCTDevice 0"
}
...
if { ("$val(traffic)" == "mix") || ("$val(traffic)" == "cbr") || ("$val(traffic)" == "poisson") } {
   ...
   ${traffic1}traffic 67 45 $val(trInterval) $appTime1
   ${traffic2}traffic 71 74 $val(trInterval) $appTime2
   if { "$val(nodeDown)" == "yes" } {
   	$ns_ at [expr $appTime2 + 3.0] "$node_(0) node-down"
   	set tmpTime [format "%.1f" [expr $appTime2 + 3.0]]
   	$ns_ at [expr $appTime2 + 3.0] "$ns_ trace-annotate \"(at $tmpTime) node down: 0\""
	$ns_ at [expr $appTime2 + 10.0] "$node_(0) node-up"
	set tmpTime [format "%.1f" [expr $appTime2 + 10.0]]
	$ns_ at [expr $appTime2 + 10.0] "$ns_ trace-annotate \"(at $tmpTime) node up: 0\""
   }
   ...
   $ns_ at $appTime1 "$node_(67) add-mark m1 blue circle"
   $ns_ at $appTime1 "$node_(45) add-mark m2 blue circle"
   $ns_ at $appTime1 "$ns_ trace-annotate \"(at $appTime1) $traffic1 traffic from node 67 to node 45\""
   $ns_ at $appTime2 "$node_(71) add-mark m3 green4 circle"
   $ns_ at $appTime2 "$node_(74) add-mark m4 green4 circle"
   $ns_ at $appTime2 "$ns_ trace-annotate \"(at $appTime2) $traffic2 traffic from node 71 to node 74\""
   if { "$val(traffic)" == "cbr" } {
   	set pktType cbr
   	set pktType2 cbr
   ...
   Mac/802_15_4 wpanNam FlowClr -p $pktType -s 67 -d 45 -c blue
   Mac/802_15_4 wpanNam FlowClr -p $pktType2 -s 71 -d 74 -c green4
}
...
if { "$val(traffic)" == "ftp" } {
   ...
   puts [format "Acknowledgement for data: %s" [Mac/802_15_4 wpanCmd ack4data]]
   ftptraffic 67 45 $appTime1
   ftptraffic 71 74 $appTime2
   Mac/802_15_4 wpanNam FlowClr -p AODV -c tomato
   Mac/802_15_4 wpanNam FlowClr -p ARP -c green
   Mac/802_15_4 wpanNam FlowClr -p MAC -c navy
   $ns_ at $appTime1 "$node_(67) add-mark m1 blue circle"
   $ns_ at $appTime1 "$node_(45) add-mark m2 blue circle"
   $ns_ at $appTime1 "$ns_ trace-annotate \"(at $appTime1) ftp traffic from node 67 to node 45\""
   Mac/802_15_4 wpanNam FlowClr -p tcp -s 67 -d 45 -c blue
   Mac/802_15_4 wpanNam FlowClr -p ack -s 45 -d 67 -c blue
   $ns_ at $appTime2 "$node_(71) add-mark m3 green4 circle"
   $ns_ at $appTime2 "$node_(74) add-mark m4 green4 circle"
   $ns_ at $appTime2 "$ns_ trace-annotate \"(at $appTime2) ftp traffic from node 71 to node 74\""
   Mac/802_15_4 wpanNam FlowClr -p tcp -s 71 -d 74 -c green4
   Mac/802_15_4 wpanNam FlowClr -p ack -s 74 -d 71 -c green4
}
...
proc stop {} {
    ...
    if { ("$val(nam)" == "wpan_demo5.nam") && ("$hasDISPLAY" == "1") } {
    	exec nam wpan_demo5.nam &
    }
}
