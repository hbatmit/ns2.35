...
#ifdef NS_DIFFUSION
static class OPPPingSenderAppClass : public TclClass {
public:
  OPPPingSenderAppClass() : TclClass("Application/DiffApp/PingSender/OPP") {}
  TclObject* create(int , const char*const*) {
    return(new OPPPingSenderApp());
  }
} class_opp_ping_sender;

void OPPPingSendDataTimer::expire(Event *) {
  a_->send();
}
...
void OPPPingSenderReceive::recv(NRAttrVec *data, NR::handle my_handle)
{
  app_->recv(data, my_handle);
}

void OPPPingSenderApp::recv(NRAttrVec *data, NR::handle )
{
...
handle OPPPingSenderApp::setupPublication()
{
  NRAttrVec attrs;
...
#ifdef NS_DIFFUSION
OPPPingSenderApp::OPPPingSenderApp() : sdt_(this)
#else
...
{
  OPPPingSenderApp *app;

  app = new OPPPingSenderApp(argc, argv);
  app->run();
