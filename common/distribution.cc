...
Distribution::Distribution( std::vector<double> value_vector ) :
	_pmf( std::vector<std::tuple<double,double>>( value_vector.size() ) )
{
	/* Append probability masses and write into new vector */
	std::transform( value_vector.begin(), value_vector.end(), _pmf.begin(), [&] (double x) { return std::tuple<double,double>( x, 1.0/value_vector.size() ); } );

	/*Sort for finding quantiles later */
	std::sort( _pmf.begin(), _pmf.end(),
	           [&] (const std::tuple<double,double> & x, const std::tuple<double,double> & y)
	           { return std::get<0>(x) < std::get<0>(y); });
}

Distribution::Distribution( std::vector<std::tuple<double,double>> pmf ) :
	_pmf( pmf )
{}

double Distribution::quantile( double location )
{
...
Distribution Distribution::compose( Distribution & other )
{
	...
	std::vector<std::tuple<double,double>> self_new_pmf(  this->get_size() );
	std::vector<std::tuple<double,double>> other_new_pmf( other.get_size() );
	std::transform( _pmf.begin(), _pmf.end(), self_new_pmf.begin(),
	                [&] ( const std::tuple<double,double> & x)
	                { return std::tuple<double,double> (std::get<0>(x), std::get<1>(x)*self_prob); } );
	std::transform( other._pmf.begin(), other._pmf.end(), other_new_pmf.begin(),
	                [&] ( const std::tuple<double,double> & x)
	                { return std::tuple<double,double> (std::get<0>(x), std::get<1>(x)*other_prob); } );

	/*Merge sort the two lists */
	std::vector<std::tuple<double,double>> merged_pmf( this->get_size() + other.get_size() );
